  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS public.users (
        ...
        id_front TEXT,
        id_back TEXT,
        CONSTRAINT users_role_check CHECK (role = ANY (ARRAY['client', 'barber', 'admin']))
      );
    `);
    ...
    `);
    console.log('✅ Tabla service_request creada');
    ...
    // Índices
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_bids_service_request_barber_status 
      ON public.bids(service_request_id, barber_id, status);
    `);
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_service_request_assigned_barber_id 
      ON public.service_request(assigned_barber_id);
    `);
